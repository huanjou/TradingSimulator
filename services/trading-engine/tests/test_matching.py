...
def create_order(
    order_id: str, side: OrderSide, type_: OrderType, quantity: str, price: str = None
) -> Order:
    return Order(
        id=order_id,
        user_id="user1",
...
def test_basic_limit_matching():
    ...
    sell_order = create_order(
        "sell_1", OrderSide.SELL, OrderType.LIMIT, "2.0", "50000.0"
    )
    ...
    assert any(
        u.order_id == "sell_1" and u.status == OrderStatus.FILLED for u in updates
    )
    assert any(
        u.order_id == "buy_1" and u.status == OrderStatus.FILLED for u in updates
    )
...
def test_partial_fill():
    ...
    sell_order = create_order(
        "sell_1", OrderSide.SELL, OrderType.LIMIT, "10.0", "50000.0"
    )
    ...
    buy_order_2 = create_order(
        "buy_2", OrderSide.BUY, OrderType.LIMIT, "8.0", "50000.0"
    )
...
def test_market_order_matching():
    ...
    engine.process_order(
        create_order("sell_1", OrderSide.SELL, OrderType.LIMIT, "2.0", "100.0")
    )
    engine.process_order(
        create_order("sell_2", OrderSide.SELL, OrderType.LIMIT, "2.0", "200.0")
    )
    ...
    taker_update = next(
        u
        for u in updates
        if u.order_id == "buy_mkt"
        and u.status
        in [OrderStatus.CANCELED, OrderStatus.FILLED, OrderStatus.PARTIALLY_FILLED]
    )
...
def test_time_priority():
    ...
    engine.process_order(
        create_order("sell_early", OrderSide.SELL, OrderType.LIMIT, "2.0", "100.0")
    )
    engine.process_order(
        create_order("sell_late", OrderSide.SELL, OrderType.LIMIT, "2.0", "100.0")
    )
    ...
    assert len(trades) == 1
    assert (
        trades[0].maker_order_id == "sell_early"
    )  # Should match with the older one first
